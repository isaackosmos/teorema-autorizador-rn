...
| F3  | Erro tipado ponta a ponta                | §7.1.3     | Auditar cada tratamento de erro: decisão sempre por `ApiError.status`, nunca por texto. **Entra aqui a dívida D12** (`CLAUDE.md §9`): seis respostas de API usam `.parse()` cru, e o `ZodError` sobe sem virar `ApiError` — o padrão a copiar é o `safeParse` + `ApiError` de `entrar`, no mesmo arquivo |
