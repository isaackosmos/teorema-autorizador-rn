...
| --- | ---------------------------------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F1  | Fallback primário → secundário → offline | §3.1       | Resolver no `tenantApi` a cada requisição, gravando o resultado em `device.serverUrlActive`; sem duplicar o teste em cada tela. **Feita em 15/09/2026:** `shared/lib/http/failover.ts` reenvia **uma vez** contra o outro endereço quando o ativo não responde, e o que responder vira o ativo — nos dois sentidos, sem espera artificial e sem poller (a retomada é na próxima requisição). Reenvia a própria requisição em vez de sondar `/v1/ping`, que além de gastar menos ida ao servidor é o único caminho legal pela §2: `shared/` não importa o `ping()` de `features/auth`. Com ela **a dívida D9 foi paga nos sítios de pré-condição de sessão** — os quatro que usavam `ApiError(0)` viraram `SessionError` (424). O `toApiError` continua cunhando 0 para o que não é `AxiosError` (um `TypeError` dentro de `api/` ainda chega ao login como "Verifique a conexão"), e isso ficou na **D15**, o login deixou de dizer "Verifique a conexão" para aparelho sem endereço, e `online: false` passou a significar o que o store promete: nem primário nem secundário responderam. Ficou anotada a **D15** (o mesmo `ApiError(0)` num quinto sítio, em `features/push`) |
| F2  | Cache offline de empresas                | §3.1       | Substituto da tabela `COMPANYS` do SQLite: persistir a lista no MMKV para o login offline                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| F3  | Erro tipado ponta a ponta                | §7.1.3     | Auditar cada tratamento de erro: decisão sempre por `ApiError.status`, nunca por texto. **Feita em 15/09/2026, e com ela a dívida D12 foi paga:** as seis respostas de API trocaram `.parse()` cru por `safeParse` + `ContractError` (502), o `queryClient` deixou de repetir o que é determinístico, e a auditoria confirmou que nenhuma decisão por texto sobrou fora da D8. Ficaram anotadas a **D13** (`parse` de entrada sobre `user` vindo do disco sem schema) e a **D14** (o 404 fabricado que confunde corpo vazio com contrato quebrado)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| F4b | Testes automatizados — hooks             | §1         | Hook de escrita: invalidação, ciclo de vida da reserva e erro tipado. Resolvido **sem** segundo runner — `@testing-library/react` + happy-dom sobre o mesmo `node --test`, com stubs de módulo nativo em `tools/test-setup.mjs`. Tela continua fora: aí sim precisaria de jest-expo                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
