...
| D12 | `auth.api.ts:105` · `auth.api.ts:121` · `empresa.api.ts:11` · `liberacoes.api.ts:24` · `:48` · `:54`                                                                  | **`.parse()` cru em seis respostas de API: o `ZodError` sobe sem virar `ApiError` e fura a promessa do §4.1.** O `parse` roda _depois_ do interceptor, então o `toApiError` não o vê. Três consequências, todas verificáveis: (1) o `queryState` renderiza `error.message`, e a de um `ZodError` é o **dump JSON** dos issues — a tela mostra JSON cru, que é justamente o que o §8 proíbe; (2) o `retry` do `queryClient` decide por `instanceof ApiError`, então um payload malformado — que é determinístico — vira **três requisições idênticas** nos **cinco que são query** (o `:121`, de `registrarAparelho`, é mutation e `mutations: { retry: false }` o poupa); (3) quem decide por `status` (§5.11) não tem status para decidir. O agravante é que **não falta padrão: ele está no mesmo arquivo** — `entrar`, `buscarEmpresaLicenciada` e `buscarEnderecosServidor` usam `safeParse` + `ApiError`, e o comentário de `entrar` já escreve a regra por extenso. Nenhum dos seis sítios tem teste de 200 malformado — mas o **formato existe**, em `use-login.test.ts:162`, justamente sobre o `login()` que já faz certo. Caso menor e diferente: `auth.api.ts:117` e `push.api.ts:20` dão `parse` na **entrada** — dado nosso, erro de programação, não desvio de contrato. É a única linha desta tabela que viola a **letra** do plano §5 item 4 em tela já marcada ✅ (A3/A6/C1/C3); fica aceita porque o efeito só aparece com payload fora de contrato, que é o cenário que o 🔒 B1 impede de observar. | **Na F3** (plano §3, "auditar cada tratamento de erro"), que é o dono natural: trocar os seis por `safeParse` + `ApiError(502, …)` copiando o código de `entrar` e o teste de `use-login.test.ts:162` — os dois modelos já existem. Achado em 14/09/2026, anotado em 15/09. |
