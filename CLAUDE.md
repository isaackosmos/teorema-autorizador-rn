...
- **Resposta 2xx que o schema recusar vira `ContractError` (502) na borda**, nunca um `ZodError`
  solto: `safeParse` e `throw new ContractError('Resposta inesperada do servidor …', data)`, como
  em `login` e `listarPendentes`. O `ZodError` nasceria **depois** do interceptor, então o
  `toApiError` não o veria — a tela receberia o dump JSON dos issues como mensagem e o
  `queryClient` repetiria três vezes um payload que não vai mudar. `.parse()` cru fica só para a
  **entrada** (`registroAparelhoRequestSchema`, `tokenPushRequestSchema`): ali o dado é nosso e
  conferido antes, e a falha seria defeito do app. Cuidado com o que **parece** nosso e veio do
  disco sem schema — é a dívida D13 do §9.
...
| Item                                                                     | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Clientes HTTP (central + tenant) com `ApiError`                          | ✅ cobre também o **200 fora do contrato**: vira `ContractError` 502 na borda, e o `queryClient` não repete o que é determinístico (F3)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Fallback primário → secundário → offline                                 | ✅ o onboarding elege `serverUrlActive` e, em runtime, o interceptor de `shared/lib/http/failover.ts` reenvia **uma vez** contra o outro endereço quando o ativo não responde; o que responder vira o ativo, nos dois sentidos, e a eleição vale para as requisições seguintes. Nenhuma tela repete o teste. **Só método idempotente é reenviado** (`GET`/`HEAD`): "sem resposta" inclui timeout, e como os dois endereços são do mesmo tenant, reenviar um `authorize` autorizaria a liberação duas vezes — quem reelege depois de uma decisão é a próxima leitura. `online: false` passou a significar o que o store promete: nem primário nem secundário responderam                                                                                                                                                                |
...
| Testes automatizados                                                     | 🟨 (`node --test` — §1. **254 casos.** Fora do React (F4): `liberacao`, `cliente`, `bordero-retorno`, `auth` e `mensagem-web`, o mapa de erro do login, `toApiError`, o guarda de origem da WebView, documento e data. Hooks de escrita (F4b): decidir, reservar/devolver, registrar aparelho e login — invalidação, ciclo de vida e erro tipado. Failover do tenant (F1): eleição do endereço vivo, não-repetição, os casos em que **não** se troca (4xx, `baseURL` do chamador, sem alternativo) e o `SessionError`. Borda de API (F3): as seis respostas de 200 fora do contrato, a política de retry do `queryClient` e o motivo de contrato no mapa de erro do login. **Ainda sem teste:** os schemas `login`, `erro-login`, `bordero-abertura` e `push`; os hooks de leitura; e **toda tela**, que precisaria do renderer do RN) |
...
| D6  | `dados-cliente.tsx` · `campos-cliente.ts` · `cliente-titulo-card.tsx` · `liberacao-resumo.tsx` · `use-analise-credito.ts` · `documento.test.ts` · `use-login.test.ts` · `auth.api.test.ts` · `empresa.api.test.ts` · `liberacoes.api.test.ts` | Nome de coluna Firebird fora de `schemas/`. Em quase todos é **comentário**, e nenhum código depende deles: é o §5.8 do plano (citar a origem da regra) cruzando com o §5.3 (o nome não sai de `schemas/`). A letra do §5.3 está violada, o espírito não. **A exceção é `use-login.test.ts`** (F4b): ali os nomes estão em código, numa fixture, porque o servidor falso devolve o payload **cru** e o teste existe para provar que o schema o traduz. Esconder os nomes atrás de um helper não os tiraria do repositório e deixaria o teste ilegível. **A F3 ampliou essa exceção para os três `*.api.test.ts`** (15/09/2026), pela mesma razão e sem inventar outra: o servidor falso devolve o payload cru, e um teste de tradução que não mostra o que entra não prova tradução nenhuma. | Precisa de uma decisão do time, não de um patch. Até lá, não crie **código** que leia coluna fora de `schemas/` — a fixture de teste é a única exceção, e só porque o que ela representa é o payload cru.                                                                                                                                                               |
...
| D13 | `auth.api.ts:125` · `push.api.ts:20` · `shared/stores/session.store.ts`                                                                                                                                                                       | O `.parse()` de **entrada** é tratado como defeito do app — mas os dois sítios rodam dentro de `mutationFn`: o TanStack captura o `ZodError` e a tela imprime `error.message`, que é o dump JSON dos issues (`passo-registro.tsx:62`). É a consequência (1) da antiga D12 sobrevivendo no único lugar que a F3 declarou seguro. Hoje inalcançável porque `use-registrar-aparelho.ts:29-39` confere cada campo antes de chamar — **menos `user.username` e `user.id`**, que vêm do `user` restaurado do disco pelo `merge` raso do `session.store.ts`, **sem schema**, contra o que o §4.8 manda. Achado pelo revisor na F3 (15/09/2026).                                                                                                                                                     | Na primeira ficha que tocar a restauração da sessão: validar o `user` do disco por schema, como `historico-usuarios.store.ts` já faz. Enquanto isso, o `.parse()` fica: converter entrada em `ContractError` seria culpar o servidor por defeito nosso.                                                                                                                 |
| D14 | `auth.api.ts` (`buscarEmpresaLicenciada`, `buscarEnderecosServidor`)                                                                                                                                                                          | O `safeParse` falha por **duas** causas e devolve a mesma coisa: o `200 {}` documentado (não licenciado — regra de negócio, `analise §7.1.8`) e um corpo **com conteúdo** que o schema recusa (desacordo de contrato). As duas viram `ApiError` 404 "Documento ou licença não encontrado", então uma troca de nome de campo no Orion apareceria como documento inexistente, sem rastro — e `companyinformation` é justamente uma das primeiras rotas a ver payload real quando o 🔒 B1 cair.                                                                                                                                                                                                                                                                                                 | Ao 🔒 B1 cair, na reverificação do Bloco A: corpo vazio (`{}`, texto vazio ou `null`) segue 404 de negócio, corpo com conteúdo recusado vira `ContractError`. Mantido agora **de propósito**: o corte depende de saber o que o Orion manda para documento sem licença, e trocar a mensagem do onboarding contra payload que ninguém viu é o que a D11 já recusou fazer. |
| D15 | `features/push/lib/registro-push.ts:59` · `shared/lib/http/errors.ts` (`toApiError`)                                                                                                                                                          | O status 0 que sobrou depois da D9. Dois sítios: o `ApiError(0, …)` para "token de push em formato inesperado", que não é rede porque ninguém tentou falar com servidor nenhum; e o fallback do `toApiError`, que cunha 0 para qualquer coisa que não seja `AxiosError` nem `ApiError` — um `TypeError` dentro de uma função de `api/` chega ao `motivoDoErroDeLogin` como `Rede` e vira "Verifique a conexão", que é o defeito que a D9 descrevia. É por isso que a F1 declara a D9 paga **nos sítios de pré-condição de sessão**, e não por inteiro.                                                                                                                                                                                                                                       | Na primeira ficha que tocar `features/push/` (o token) ou o mapa de erro (o fallback). O que decide o segundo é escolher um status para "erro que não veio da rede nem do servidor" — mudança de blast radius maior que a F1, porque `isNetworkError` é lido por duas telas.                                                                                            |
| D17 | `features/web-systems/components/web-system-view.tsx:29,64`                                                                                                                                                                                   | A F1 fez `device.serverUrlActive` mudar **em runtime**; a WebView lê esse valor por seletor e o usa em `source` e em `originWhitelist`. Um failover disparado por qualquer requisição de fundo — a invalidação da fila por push, por exemplo — enquanto um web system está aberto recarrega a página: o borderô em preenchimento se perde e o handshake recomeça. O Bloco D foi escrito quando o endereço era imutável com o app aberto, e essa premissa deixou de valer. Achado pelo revisor ao fechar a F1 (15/09/2026); não corrigido aqui porque fixar a URL é decisão do Bloco D, não do transporte.                                                                                                                                                                                    | Na primeira ficha que tocar `web-systems/`: fixar a URL no mount com `useRef`, como o `contexto` do handshake já faz — é o mesmo padrão de valor lido uma vez (§4.11).                                                                                                                                                                                                  |
