...
npm run test:watch
node --import ./tools/ts-alias-hook.mjs --test "src/features/auth/**/*.test.ts"   # um recorte
```
...
- **Exige Node ≥ 22.18** (o `engines` do `package.json`): o runner depende do type-stripping
  ...
  **não verificado**: a suíte só rodou em 24.x até aqui. É por isso que `ApiError` não usa _parameter properties_ — o strip-only recusa a sintaxe,
  e o comentário no arquivo diz para não encolher de volta.
- **O que dá para testar é o que roda fora do React.** Schema Zod, tradução de payload, mapa de
  erro, formatação. Hook fica de fora: precisa de renderizador, e `session.store` puxa
  `react-native-mmkv`, que é nativo — é a ficha F4b.
- **Os tipos do Node ficam fora do app.** `tsconfig.json` exclui `**/*.test.ts` e o
  `tsconfig.test.json` é quem tem `types: ["node"]`; por isso `npm run typecheck` roda os dois. Sem
  essa separação, `Buffer.from(...)` numa tela passaria no `tsc` e quebraria no aparelho.
...
| Testes automatizados                                                     | 🟨 (`node --test` instalado na F4 — §1. 173 casos cobrem `liberacao`, `cliente`, `bordero-retorno`, `auth` e `mensagem-web` schema a schema, o mapa de erro do login, `toApiError`, o guarda de origem da WebView, documento e data. **Ainda sem teste:** `login`, `erro-login`, `bordero-abertura`, `empresa` e `push` — e tudo que precisa de React: hook de mutação e ciclo de vida da reserva são a ficha F4b) |
...
| D6  | `dados-cliente.tsx` · `campos-cliente.ts` · `cliente-titulo-card.tsx` · `liberacao-resumo.tsx` · `use-analise-credito.ts` · `documento.test.ts` | Nome de coluna Firebird **em comentário** fora de `schemas/`. Nenhum código depende deles: é o §5.8 do plano (citar a origem da regra) cruzando com o §5.3 (o nome não sai de `schemas/`). A letra do §5.3 está violada, o espírito não.                                                                                                                                                                                                                                                                                                                                                                                             | Precisa de uma decisão do time, não de um patch. Até lá, não crie **código** que leia coluna fora de `schemas/`.                                                                                                |
